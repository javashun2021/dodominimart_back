...
一个**商户(brand)** 下可以有**多个门店/分店(outlet)**，多门店**共用同一套商品、库存与品牌信息**；
用户在「附近」看到的是**门店**，下单由**就近门店**发货。

> **已拍板(2026-07)：库存统一，不按门店独立。** 分店之间**只有地址/坐标不同**，
> 商品和库存都是商户级共享的一个池。因此**永远不建 per-branch 库存表**，`mall_store`
> 分店行本质就是「同一商户多一个 location」。—— 这让方案退化成纯粹的「加地址」，很轻。
...
1. ~~库存~~ **已定：库存统一，不按门店独立**。`mall_product.stock` 保持商户级共享一个池，
   不建 `mall_product_stock`。分店只影响「从哪个地址发货/自提」，不影响可售数量。
